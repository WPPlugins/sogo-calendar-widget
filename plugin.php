        ...
        if ($meta_filed != '') {
            if (!empty($r)) {
                foreach ($r as $p) {
                    ...
                    if ($d) {
                        $d = $field_format =='timestamp'? $d: strtotime($d);
                        $events[date('d',$d)][] = $p;
                    }
...
//         var_dump($events);
        ...
        $calendar .= "<div class='month-nav'   >
            <a href='#' data-month='1-$month-$year' id='cal-next'  > < </a>
            <a data-month='1-$month-$year' href='#' id='cal-prev' > > </a></div>";
        /* draw table */
